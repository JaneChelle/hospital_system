...
public interface DrugMapper extends Mapper<Drug> {
    ...
    @Select("SELECT * FROM tb_drug WHERE drug_name LIKE CONCAT('%',#{0},'%') or pinyin_code LIKE CONCAT('%',#{0},'%')  limit 10")
    List<Drug> keyWordName(String drugName);                     //搜索框提示词

    @Select("SELECT * FROM tb_drug WHERE drug_name LIKE CONCAT('%',#{0},'%') or drug_code LIKE CONCAT('%',#{0},'%') ")
    ...
    @Select("SELECT * FROM tb_drug ORDER BY (drug_code) DESC")
